...
public final class IKeyByTask extends IExecutorContextTask {
    ...
    public IKeyByTask(String name, IExecutor executor, ISource function) {
    ...
    public void run(ITaskContext context) throws IgnisException {
        LOGGER.info(log() + "keyBy started");
        try {
            executor.getGeneralModule().keyBy(function);
        } catch (IExecutorException ex) {
            throw new IExecutorExceptionWrapper(ex);
        } catch (TException ex) {
            throw new IgnisException(ex.getMessage(), ex);
        }
        LOGGER.info(log() + "keyBy finished");
    }
