...
public class MesosSingularity implements IScheduler {
    public static final String NAME = "mesos_singularity";

    private static final org.slf4j.Logger LOGGER = LoggerFactory.getLogger(MesosSingularity.class);
    ...
    public MesosSingularity(String url) {
        List<String> hosts;
        if (url.contains(",")) {
    ...
    public List<IContainerInfo> getExecutorContainers(List<String> ids) throws ISchedulerException {
        ...
        for (int i = 0; i < ids.size(); i++) {
            ...
            result.add(getContainer(ids.get(i)));
        }
        return result;
    }
